...
function getSortedOccurrences(array: string[]): Occurrency[] {
    const buildArray: [string, number][] = Object.entries(array.reduce((res, cur) => {
    ...
    const mapAr = new Map<string, number>(sortedArray);
    const result = [];
    for (let entry of mapAr.entries()) {
        let el: Occurrency = { str: entry[0], count: entry[1] };
        result.push(el);
    }
    return result;
}
/*********************************************************************************************/
const set = new Set<number>([1, 1, 23, 55, 70]);
const set1 = new Set<number>([2, 23, 70, 70, 48]);
